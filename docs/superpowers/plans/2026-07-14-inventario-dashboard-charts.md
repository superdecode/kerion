...
**Architecture:** All changes live in one backend route file (`backend/src/modules/inventory/routes/dashboard.routes.js`, extending its existing `Promise.all` with new queries under new `graficas` keys) and one frontend file (`frontend/src/modules/Dashboard/pages/InventarioDashboard.jsx`). A small shared bucketing util is extracted from `surtido.dashboard.routes.js` so the new adaptive trend chart doesn't duplicate that logic.
