...
async function getCreds(tenantId) {
  ...
  if (cached && Date.now() - cached.at < CRED_TTL_MS) return cached.creds
...
export function invalidateCredCache(tenantId) {
  if (tenantId) _credCache.delete(tenantId)
 ...
 */
export async function wmsPost(tenantId, endpoint, dataPayload) {
 ...
 */
export async function wmsPostAll(tenantId, endpoint, baseData = {}, pageSize = 100) {
  const allItems = []
  let page = 1

  while (true) {
    const payload = [{ ...baseData, page, pageSize }]
    const res = await wmsPost(tenantId, endpoint, payload)
...
export async function cacheSet(tenantId, key, data, ttlSeconds) {
    ...
    `INSERT INTO wms_cache (key, data, expires_at, tenant_id)
     ...
     ON CONFLICT (tenant_id, key) DO UPDATE
       SET data = EXCLUDED.data,
           ...
           created_at = now()`,
    [key, JSON.stringify(data), String(ttlSeconds), tenantId]
  )
}
 ...
 */
export async function getInventoryMap(tenantId) {
  const CACHE_KEY = 'inventory:full'
  const cached = await cacheGet(tenantId, CACHE_KEY)
  if (cached) return cached

  const items = await wmsPostAll(tenantId, '/integratedInventory/pageOpen', {})
  ...
  await cacheSet(tenantId, CACHE_KEY, map, 600) // 10 minutes
  return map
}
