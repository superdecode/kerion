...
router.get('/parametros',
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        guias_por_tarima: cfg.guias_por_tarima ?? 100,
        peso_habilitado: cfg.peso_habilitado ?? false,
        unidad_peso: cfg.unidad_peso ?? 'kg',
      })
    } catch (error) {
...
router.put('/parametros',
  ...
  async (req, res) => {
    try {
      const { guias_por_tarima, peso_habilitado, unidad_peso } = req.body
      ...
      const unidad = ['kg', 'g', 'lb'].includes(unidad_peso) ? unidad_peso : 'kg'
      ...
      const newCfg = { ...existingCfg, guias_por_tarima: gpt, peso_habilitado: pesoHab, unidad_peso: unidad }
      await req.tQuery(
      ...
      res.json({ guias_por_tarima: gpt, peso_habilitado: pesoHab, unidad_peso: unidad })
    } catch (error) {
