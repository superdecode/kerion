...
import {
  FileText, Printer, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Eye, Trash2,
  X, Clock, CheckCircle, XCircle, AlertTriangle,
  Building2, Search, Plus, ChevronDown, ChevronUp,
  Download, ArrowRight, Radio, FilePlus, Copy, User, Package
} from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="sticky top-0 z-[5] bg-white/80 backdrop-blur-2xl border-b border-warm-100/60 px-5 py-2 space-y-2">
          ...
          <div className="flex items-center gap-1.5 bg-warm-50 border border-warm-200 rounded-xl px-3 py-1.5">
            ...
            <input type="date" value={filters.fecha_desde}
                onChange={e => { isRollingDateRangeRef.current = false; setDatePreset(''); setFilters(f => ({ ...f, fecha_desde: e.target.value })); setPage(1) }}
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1.5 h-10 bg-warm-50 border border-warm-200 rounded-xl px-3 min-w-[220px] transition-all focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-primary-100 focus-within:shadow-sm">
                ...
                <input type="text" value={folioSearchInput}
                  ...
                  className="h-full text-xs outline-none bg-transparent text-warm-700 flex-1 focus-visible:outline-none focus-visible:ring-0 focus-visible:ring-offset-0" />
                {folioSearchInput && (
                  ...
                  <tbody className="divide-y divide-warm-50">
                    {folios.map(row => (
                      <tr key={row.id}
                        ...
                        className={`table-row group ${canViewFolios ? 'cursor-pointer' : 'cursor-default'}`}>
                        <td className="table-cell">
                          <div className="flex items-center gap-1.5">
                            <span className="code-main">{row.folio_numero}</span>
                            <button
      ...
      <Modal
        ...
        footer={wizStep < 4 ? (
          ...
          <>
            <button onClick={() => setShowWizard(false)} className="btn-secondary">{t('common.close')}</button>
            {createdFolio && (
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-2.5 group/fheader">
            <span className="code-main">{detailFolio ? detailFolio.folio_numero : 'Cargando...'}</span>
            {detailFolio && (
              <button
        ...
        footer={detailFolio && (
          <>
            ...
            <button onClick={() => { setSelectedFolioId(null); setExpandedDetailTarima(null) }} className="btn-secondary">
              {t('common.close')}
            </button>
          </>
        )}
                      ...
                      <table className="w-full text-xs">
                        ...
                        <tbody className="divide-y divide-warm-50">
                          {detailGuiasPage.map((g, i) => (
                            <tr key={g.codigo_guia + i} className="table-row">
                              <td className="px-3 py-2 text-warm-400">{(detailGuiaPage - 1) * DETAIL_GUIAS_PER_PAGE + i + 1}</td>
                              <td className="px-3 py-2"><span className="code-main">{g.codigo_guia}</span></td>
                              <td className="px-3 py-2"><span className="code-main">{g.tarima_codigo}</span></td>
                    ...
                    {detailGuiaPages > 1 && (
                      <div className="flex items-center justify-between gap-2 px-2 pt-3">
                        <p className="text-xs text-warm-400">
                          {t('common.pagination.page')} {detailGuiaPage} {t('common.pagination.of')} {detailGuiaPages} · {detailGuias.length} guías
                        </p>
                        <div className="flex items-center gap-1.5">
                                      ...
                                      <tbody>
                                        {tGuias.map((g, i) => (
                                          <tr key={g.codigo_guia + i} className="border-b border-warm-50 last:border-0">
                                            ...
                                            <td className="pr-3"><span className="code-main">{g.codigo_guia}</span></td>
      ...
      <Modal isOpen={!!cancelTarget} onClose={() => setCancelTarget(null)}
        ...
        footer={<>
          <button onClick={() => setCancelTarget(null)} className="btn-secondary">{t('common.close')}</button>
          ...
          {deleteTarget && (
            <div className="p-3 rounded-xl bg-warm-50 border border-warm-200 space-y-1">
              <div className="flex items-center justify-between">
                ...
                <span className="code-main">{deleteTarget.folio_numero}</span>
              </div>
              <div className="flex items-center justify-between">
      ...
      <div className="space-y-4">
        ...
        <div className="max-h-60 overflow-y-auto rounded-xl border border-warm-100 scrollbar-thin">
          <table className="w-full text-xs">
            ...
            <tbody className="divide-y divide-warm-50">
              {selected.map(tar => (
                <tr key={tar.id} className="table-row">
                  <td className="px-3 py-2"><span className="code-main">{tar.codigo}</span></td>
  ...
  if (step === 4 && createdFolio) {
    return (
      <div className="space-y-4 text-center py-4">
        ...
        <div>
          <p className="code-main text-2xl">{createdFolio.folio_numero}</p>
