            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {MODULE_LIMITS.map(({ icon: Icon, color, bg, border, label, sub, field }) => (
                  <div key={field} className={`bg-gray-800/40 border rounded-xl p-2.5 ${border}`}>
                    <div className="flex items-center gap-1.5 mb-2">
                      <div className={`w-6 h-6 rounded-md ${bg} border ${border} flex items-center justify-center flex-shrink-0`}>
                        <Icon className={`w-3 h-3 ${color}`} />
                      </div>
                      <div className="min-w-0">
                        <p className={`text-[11px] font-semibold leading-tight ${color} truncate`}>{label}</p>
                        <p className="text-gray-600 text-[9px] leading-tight truncate">{sub}</p>
                      </div>
                    </div>
                    <input
                      type="number"
                      value={form[field]}
                      onChange={set(field)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-2 py-1.5 text-white text-xs focus:outline-none focus:border-blue-500 transition-colors"
                      placeholder="∞"
                    />
                  </div>
                ))}
              </div>
              <p className="text-gray-600 text-[10px]">Vacio = sin limite (NULL en BD).</p>
              <div className="pt-1 border-t border-gray-800">
                <p className="text-gray-500 text-[10px] uppercase tracking-wider mb-1.5">Modulos incluidos</p>
                <div className="flex flex-wrap gap-1">
                  {MODULE_CATALOG.map(module => (
                    <span
                      key={module.code}
                      className={`px-1.5 py-0.5 rounded-full border text-[9px] font-medium ${module.bg} ${module.border} ${module.color}`}
                    >
                      {module.shortName}
                    </span>
                  ))}
