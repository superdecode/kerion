    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
      ...
      <div className="grid grid-cols-2 gap-2">
        {ALL_MODULE_CODES.map(code => {
          ...
          return (
            <div key={code} className="flex items-center justify-between px-3 py-2 bg-gray-800/40 rounded-lg border border-gray-700/40 gap-2 min-w-0">
              <p className="text-white text-xs font-medium truncate">{MODULE_LABELS[code]}</p>
              <button
                onClick={() => toggle(code, enabled)}
                disabled={isSaving}
                title={enabled ? `Activo${m?.enabled_at ? ` desde ${fmtDate(m.enabled_at)}` : ''}` : `Inactivo${m?.disabled_at ? ` desde ${fmtDate(m.disabled_at)}` : ''}`}
                className={`flex-shrink-0 flex items-center gap-1 text-xs px-2 py-1 rounded-md border transition-colors disabled:opacity-50 ${
                  enabled
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          />
          <SummaryCard
            icon={Database} label="Peso base de datos"
            value={stats ? fmt(stats.total_rows_estimate) : '—'}
            sub="registros estimados"
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
          <p className="text-gray-500 text-xs uppercase tracking-wider mb-3">Actividad por módulo</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 items-stretch">
        ...
        <div className="flex flex-col gap-5 h-full">
          ...
          {(() => {
            ...
            const errCount = provisioning_log.filter(l => l.status === 'error').length
            return (
              <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                ...
                >
                  <span className="flex items-center gap-2 text-white font-semibold text-sm">
                    ...
                    Log de provisioning
                    {provTotal > 0 && (
                      <span className="text-xs text-gray-500 font-normal">{provTotal} entradas</span>
                    )}
                    {errCount > 0 && (
                      <span className="text-xs bg-red-500/20 text-red-400 border border-red-500/30 rounded-full px-2 py-0.5">{errCount} error{errCount !== 1 ? 'es' : ''}</span>
                    )}
                  </span>
                  <ChevronRight className={`w-4 h-4 text-gray-500 transition-transform ${provOpen ? 'rotate-90' : ''}`} />
                </button>
                ...
                {provOpen && (
                  <>
                    <div className="px-4 pb-3 border-t border-gray-800/60">
                      {provTotal === 0 ? (
                        <p className="text-gray-500 text-sm pt-3">Sin registros de provisioning</p>
                      ) : (
                        <div className="space-y-0.5 max-h-56 overflow-y-auto pt-2">
                          {provPaged.map(l => (
                            <div key={l.id} className="flex items-center gap-3 py-1.5 border-b border-gray-800/60 last:border-0 text-xs">
                    ...
                    {provTotal > PROV_PAGE_SIZE && (
                      <div className="border-t border-gray-800 px-4 py-2.5 flex items-center justify-between">
                        <p className="text-xs text-gray-500">
                          {provStart + 1}–{provStart + provPaged.length} de {provTotal}
                        </p>
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => setProvPage(p => Math.max(1, p - 1))}
                            disabled={provSafePage <= 1}
                            className="p-1.5 rounded-lg border border-gray-700 bg-gray-800 text-gray-400 hover:text-white hover:bg-gray-700 disabled:opacity-40 transition-colors"
                          >
                            <ChevronLeft className="w-3.5 h-3.5" />
                          </button>
                          <span className="text-gray-400 text-xs">{provSafePage}/{provTotalPages}</span>
                          <button
