...
export default function EntradaDetalle() {
  ...
  const canExport = hasPermission('devoluciones.entradas', 'actualizar')
  const canCancelRecord = sesion?.estado === 'en_proceso' && canDelete
      ...
      <Header
        ...
        actions={
          <div className="flex gap-2">
            {sesion.estado === 'confirmado' && canExport && (
                    ...
                    {paginatedItems.map((item, idx) => (
                      <Fragment key={item.id}>
                        {(() => {
                          ...
                          return (
                        <tr
                          className={`border-b border-warm-50 table-row cursor-pointer group ${
