  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Filter bar */}
        <div data-tour="tarimas-filtros" className="sticky top-0 z-[5] bg-white/80 backdrop-blur-2xl border-b border-warm-100/60">
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2">
            ...
            <div className="col-span-2 sm:col-span-1 flex items-center gap-2">
              <div ref={guiaSearchRef} className="relative flex-1">
                <form
                  onSubmit={(e) => { e.preventDefault(); const q = guiaSearchInput.trim(); setGuiaSearch(q); setPage(1); if (q) searchGuias(q) }}
                  className={`flex items-center gap-1.5 h-10 bg-warm-50 border rounded-xl px-3 min-w-[220px] w-full transition-all focus-within:border-primary-400 focus-within:ring-2 focus-within:ring-primary-100 focus-within:shadow-sm ${guiaSearchOpen ? 'border-primary-400 bg-white' : 'border-warm-200'}`}
                >
                      ...
                      {tarimas.map(row => (
                        <tr key={row.id}
                          ...
                          <td className="table-cell text-warm-500 text-xs">
                            ...
                            <span className="ml-1 text-warm-400">{fmtTimeShort(row.fecha_inicio)}</span>
                          </td>
